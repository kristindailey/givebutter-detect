...
- [ ] PostgreSQL 16 via `docker-compose.yml` on `:5432` — **TO DO (decided: add Docker)**: app currently runs against a **local** Postgres (`.env` → `pgsql`, db `givebutter_detect`, user `kristindailey`, no password) which works, but we're adding `docker-compose.yml` for reviewer portability (`docker compose up`). Will need `.env` DB creds aligned to the compose service.
