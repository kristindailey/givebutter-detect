    ...
    return (
        <li className="flex items-center gap-4 border-b border-border px-6 py-4 last:border-b-0">
            ...
            <div className="min-w-0 flex-1">
                <p className="font-heading font-semibold text-brand-black">{label}</p>
                ...
                <div className="w-full max-w-xl">
                    <header className="mb-8 text-center">
                        <p className="font-logo text-3xl font-extrabold text-brand-black">
                            Givebutter <span className="text-brand-blue">Detect</span>
                        </p>
                    ...
                    <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
                        <div
                            className={`px-6 py-4 font-heading font-semibold ${
                                allOk ? 'bg-brand-yellow text-brand-black' : 'bg-muted text-muted-foreground'
                            }`}
                        >
                            {allOk ? 'All systems go — stack is wired end-to-end' : 'Checking stack…'}
                        </div>
                        <ul>
                            {checks.map((check) => (
                                <CheckRow key={check.key} ok={check.ok} label={check.label} detail={check.detail} />
                            ))}
                            <CheckRow
                                ok={hydrated}
                                label="React / Vite hydration"
                                detail={hydrated ? 'Client bundle mounted and interactive' : 'Waiting for client…'}
                            />
                        </ul>
                    </div>

                    <p className="mt-6 text-center text-sm text-muted-foreground">
                        Gate before feature work — all rows green confirms the stack.
                    </p>
                </div>
            </div>
        </>
